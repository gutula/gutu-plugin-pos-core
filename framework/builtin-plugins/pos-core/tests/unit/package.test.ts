...
import { businessFlowDefinitions, openPosSession, recordPosReceipt, closePosSession, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("pos-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("pos.sessions.open");
    expect(manifest.publicEvents).toContain("pos.session-opened.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof openPosSession).toBe("function");
    expect(typeof recordPosReceipt).toBe("function");
    expect(typeof closePosSession).toBe("function");
