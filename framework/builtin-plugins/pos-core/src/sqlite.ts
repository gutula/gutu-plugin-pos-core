export type PosCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildPosCoreSqliteMigrationSql(options: PosCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "pos_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getPosCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getPosCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildPosCoreSqliteRollbackSql(options: PosCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "pos_core_");
  return [
...
export function getPosCoreSqliteLookupIndexName(tablePrefix = "pos_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getPosCoreSqliteStatusIndexName(tablePrefix = "pos_core_"): string {
