export type PosCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildPosCoreMigrationSql(options: PosCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "pos_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getPosCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getPosCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildPosCoreRollbackSql(options: PosCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "pos_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "pos_core";
  return [
...
export function getPosCoreLookupIndexName(): string {
  return "pos_core_primary_lookup_idx";
}

export function getPosCoreStatusIndexName(): string {
  return "pos_core_exception_status_idx";
}
